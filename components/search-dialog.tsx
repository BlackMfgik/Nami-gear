...
function SearchDialogContent({ products, onClose, onSelect }: { products: Product[]; onClose: () => void; onSelect: (product: Product) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/45 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="mx-auto mt-[5vh] flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
        <div className="flex items-center gap-3 border-b border-line p-4 sm:p-5">
          <Search className="size-5 text-muted" />
          <input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-base outline-none" placeholder="Пошук товарів…" aria-label="Пошук товарів" />
        ...
        <div className="overflow-y-auto p-4 sm:p-6">
          {results.length ? (
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {results.map((product) => <ProductCard key={product.id} product={product} onSelect={(selected) => { onSelect(selected); onClose(); }} />)}
            </div>
